#!/usr/bin/env python3
"""Automated integration tests for minimal Lambda pipeline.

Tests:
1. Happy path - valid schema flows through pipeline
2. Failure path - schema mismatch causes pipeline failure
3. Observability - correlation IDs appear in all logs
"""
...
def test_observability(stack_outputs: dict, happy_correlation_id: str = None) -> bool:
    """Test 3: Correlation IDs in logs."""
    print("\n" + "=" * 60)
    print("TEST 3: Observability")
    print("=" * 60)

    if not happy_correlation_id:
        print("⚠ Skipping observability test (no correlation ID from happy path)")
        return True

    logs_client = boto3.client("logs")

    # Check Ingester logs for correlation ID
    ingester_log_group = f"/aws/lambda/{stack_outputs['IngesterFunctionName']}"
    try:
        response = logs_client.filter_log_events(
            logGroupName=ingester_log_group,
            startTime=int((time.time() - 600) * 1000),
            filterPattern=happy_correlation_id,
        )

        if response["events"]:
            print(f"✓ Ingester logs contain correlation ID: {happy_correlation_id}")
        else:
            print("✗ Ingester logs missing correlation ID")
            return False
    except Exception as e:
        print(f"✗ Failed to check Ingester logs: {e}")
        return False

    # Check Mock DAG logs for correlation ID
    mock_dag_log_group = f"/aws/lambda/{stack_outputs['MockDagFunctionName']}"
    try:
        response = logs_client.filter_log_events(
            logGroupName=mock_dag_log_group,
            startTime=int((time.time() - 600) * 1000),
            filterPattern=happy_correlation_id,
        )

        if response["events"]:
            print(f"✓ Mock DAG logs contain correlation ID: {happy_correlation_id}")
        else:
            print("✗ Mock DAG logs missing correlation ID")
            return False
    except Exception as e:
        print(f"✗ Failed to check Mock DAG logs: {e}")
        return False
...
def main():
    """Run all tests."""
    print("=" * 60)
    print("Minimal Lambda Pipeline - Integration Tests")
    print("=" * 60)
    print()
    ...
    print("✓ Stack: TracerUpstreamDownstreamTest")
    print(f"  Mock API: {stack_outputs.get('MockApiUrl', 'N/A')}")
    print(f"  Ingester: {stack_outputs.get('IngesterFunctionName', 'N/A')}")
    print(f"  Mock DAG: {stack_outputs.get('MockDagFunctionName', 'N/A')}")
    print()

    # Run tests
    happy_passed = test_happy_path(stack_outputs)
    failure_passed = test_failure_path(stack_outputs)
    observability_passed = test_observability(stack_outputs)
    ...
    print(f"  Observability:  {'PASSED' if observability_passed else 'FAILED'}")
    print()

    if all([happy_passed, failure_passed, observability_passed]):
